...
def analyze(infos):
    ...
    if not os.path.exists("q4_output"):
        os.makedirs("q4_output")
    mean_info.to_csv("q4_output/mean_info.csv")

    plt.figure(figsize=(12, 7))
    plt.plot(mean_info.index, mean_info["max_fitness"], label="Max Fitness", color="red")
    plt.xlabel("Number of Generations")
    plt.ylabel("Maximum Fitness")
    plt.title("Maximum Fitness Across Generations")
    plt.legend()
    plt.grid(True)
    plt.savefig("q4_output/max_fitness.png")
    plt.close()
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig("q4_output/fitness_over_generations.png")
    plt.close()
