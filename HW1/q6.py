...
def analyze(infos):
    ...
    if not os.path.exists("q6_output"):
        os.makedirs("q6_output")
    mean_info.to_csv("q6_output/mean_info.csv")
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig("q6_output/max_fitness.png")
    plt.close()
    
    plt.figure(figsize=(10, 6))
    plt.plot(mean_info.index, mean_info["mean_fitness"], label="Average Fitness")
    plt.plot(mean_info.index, mean_info["max_fitness"], label="Maximum Fitness")
    plt.plot(mean_info.index, mean_info["min_fitness"], label="Minimum Fitness")
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig("q6_output/fitness_over_generations.png")
    plt.close()
